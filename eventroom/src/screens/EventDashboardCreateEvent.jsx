// screens/EventDashboardCreateEvent.jsx – Final version: no list/link/quote, active states
...
import {
  ...
  FaFileImage, FaChevronLeft, FaChevronRight, FaCheck
} from "react-icons/fa";
...
const EventDashboardCreateEvent = () => {
  ...
  const [posterImagePreview, setPosterImagePreview] = useState(
    'https://images.unsplash.com/photo-1540575861501-7cf05a4b125a?auto=format&fit=crop&w=600&q=80'
  );
  ...
  const execCommand = (command, value = null) => {
    ...
    updateActiveStates(); // update after command
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.isVirtual && !formData.location.trim() && !formData.venue.trim()) {
    ...
    } else if (formData.isPaid && (!formData.price || Number(formData.price) <= 0)) {
  ...
  // ===== TOOLBAR (SIMPLIFIED WITH ACTIVE STATES) =====
  const renderToolbar = () => {
      ...
      <div className="lg:col-span-2 space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-gray-900">Event Images <span className="text-red-500">*</span> <span className="text-xs text-gray-400 font-normal">({images.length}/10)</span></h2>
          </div>
          {images.length < 10 && (
            ...
            <div className="grid grid-cols-3 gap-3">
              {imagePreviews.map((preview, index) => (
                <div key={index} className="relative group aspect-square">
                  <img src={preview} alt="" className="w-full h-full object-cover rounded-xl border border-gray-200" />
                  ...
                  {index === 0 && <span className="absolute bottom-1 left-1 bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded">Cover</span>}
                </div>
              ))}
            </div>
          )}
      ...
      <div className="hidden lg:block space-y-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex items-start gap-3"><FaImage className="text-blue-600 mt-0.5" /><div><p className="text-gray-500 text-xs">Images</p><p className="text-gray-900">{images.length} uploaded</p></div></div>
          </div>
          <div className="mt-4 pt-4 border-t border-gray-100">
            <button onClick={handleSubmit} disabled={isCreating} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-all shadow-sm hover:shadow-md disabled:opacity-50 text-sm">
    ...
    <div className="sm:hidden">
      ...
      <div className="flex items-center gap-3 mt-4 px-4">
        ...
        {currentStep === SECTIONS.length - 1 ? (
          <button type="button" onClick={handleSubmit} disabled={isCreating} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium text-sm transition-all shadow-sm hover:shadow-md disabled:opacity-50">
            <FaCheck className="text-xs" /> {isCreating ? "Publishing..." : "Publish"}
          </button>
        ) : (
        ...
        return (
          <div>
            ...
            {previewMode ? (
              ...
              <div ref={editorRef} contentEditable suppressContentEditableWarning className="min-h-[120px] p-4 border border-gray-200 rounded-b-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 leading-relaxed text-sm bg-white" onInput={(e) => setDescription(e.currentTarget.innerHTML)} onFocus={ensureEditorReady} onClick={ensureEditorReady} />
            )}
          </div>
        );
      case 'media':
        return (
          <div>
            <div className="flex items-center gap-2 mb-4"><span className="text-xs text-gray-400">({images.length}/10)</span></div>
            {images.length < 10 && (
              <label className="block w-full cursor-pointer mb-4">
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-blue-500 transition-colors bg-gray-50">
            ...
            {imagePreviews.length > 0 && (
              <div className="grid grid-cols-3 gap-3">
                {imagePreviews.map((preview, index) => (
                  <div key={index} className="relative group aspect-square">
                    <img src={preview} alt="" className="w-full h-full object-cover rounded-xl border border-gray-200" />
                    <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100"><FaTimes className="text-xs" /></button>
                    {index === 0 && <span className="absolute bottom-1 left-1 bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded">Cover</span>}
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white rounded-none sm:rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-4 sm:mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <button onClick={() => navigate("/dashboard/events")} className="p-2 hover:bg-gray-100 rounded-xl transition-colors"><FaArrowLeft className="text-gray-600 text-sm" /></button>
                <div><h1 className="text-xl sm:text-2xl font-bold text-gray-900">Create Event</h1><p className="text-sm text-gray-500 hidden sm:block">Fill in the details to publish your event</p></div>
              </div>
              <button onClick={handleSubmit} disabled={isCreating} className="flex items-center justify-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-all shadow-sm hover:shadow-md disabled:opacity-50 text-sm sm:text-base w-full sm:w-auto">
                <FaSave className="text-sm" /> {isCreating ? "Publishing..." : "Publish Event"}
              </button>
            </div>
          </div>

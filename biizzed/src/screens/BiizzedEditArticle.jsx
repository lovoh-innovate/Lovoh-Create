...
const BiizzedEditArticle = () => {
  ...
  useEffect(() => {
    if (articleData) {
      ...
      setTags(Array.isArray(article.tags) ? article.tags : (article.tags ? article.tags.split(',') : []));
  ...
  const handleCoverImageUpload = (e) => {
    ...
    setNewCoverImagePreviews(prev => [...prev, ...newPreviews]);
  ...
  const removeExistingCoverImage = (index) => {
    const urlToRemove = existingCoverImages[index];
    setExistingCoverImages(prev => prev.filter((_, i) => i !== index));
    setKeepImageUrls(prev => prev.filter(url => url !== urlToRemove));
  };

  const removeNewCoverImage = (index) => {
    URL.revokeObjectURL(newCoverImagePreviews[index]);
    setNewCoverImagePreviews(prev => prev.filter((_, i) => i !== index));
    setNewCoverImages(prev => prev.filter((_, i) => i !== index));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const successMsg = status === 'published' && !comingSoon
        ...
        : comingSoon
        ? 'Article updated as "Coming Soon" 📅'
        : 'Article draft updated 📝';
      toast.success(successMsg);
      navigate(`/articles/${result.slug}`);
    } catch (error) {
      console.error('Update article error:', error);
      const message = error?.data?.message || 'Failed to update article';
      toast.error(message);
  ...
  useEffect(() => {
    return () => {
      newCoverImagePreviews.forEach(url => URL.revokeObjectURL(url));
    };
  }, [newCoverImagePreviews]);
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-sm text-center max-w-md">
          ...
          <p className="text-gray-500">The article you're trying to edit doesn't exist or you don't have permission.</p>
          <button
            onClick={() => navigate('/articles')}
            className="mt-4 px-6 py-2 bg-[#1B3766] text-white rounded-xl hover:bg-[#142952]"
          >
            Go Back
          </button>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-6">
            ...
            <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {existingCoverImages.map((url, index) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => removeExistingCoverImage(index)}
              ...
              <button
                ...
                className="w-full py-3 rounded-xl border-2 border-dashed border-gray-300 hover:border-[#1B3766] transition-colors flex items-center justify-center gap-2 text-gray-500 hover:text-[#1B3766]"
              >
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                You can add/remove cover images, and insert images anywhere in the content using the image button,
